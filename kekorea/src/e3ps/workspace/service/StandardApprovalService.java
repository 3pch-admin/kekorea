...
public class StandardApprovalService extends StandardManager implements ApprovalService {
	...
	public void submitApp(Persistable per, Map<String, Object> param) throws WTException {
		...
		try {
			...
			// 결재 이름
			String name = ApprovalHelper.manager.getLineName(per);
			...
			if (isAgree) {
				master.setState(ApprovalHelper.LINE_AGREE_STAND);
			} else {
			...
			if (isAgree) {
				...
				for (int i = 0; i < agreeList.size(); i++) {
					...
					// 하였습니다.");
					agreeLine.setType(ApprovalHelper.AGREE_LINE);
					agreeLine.setRole(ApprovalHelper.WORKING_AGREE);
					agreeLine.setStartTime(startTime);
					agreeLine.setState(ApprovalHelper.LINE_AGREE_STAND);
			...
			for (int i = 0; receiveList != null && i < receiveList.size(); i++) {
				...
				receiveLine.setType(ApprovalHelper.RECEIVE_LINE);
				receiveLine.setReads(false);
				receiveLine.setSort(0);
				receiveLine.setRole(ApprovalHelper.WORKING_RECEIVE);
				// 수신 바로 시작
	...
	public Map<String, Object> approvalAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			ArrayList<ApprovalLine> appLines = ApprovalHelper.manager.getAppLines(master);
			...
			// 0 을 기준..
			boolean isLastAppLine = ApprovalHelper.manager.isLastAppLine(master, 0);
			if (isLastAppLine) {
				master.setCompleteTime(completeTime);
				master.setState(ApprovalHelper.MASTER_APPROVAL_COMPLETE);
	...
	private void setProjectRoleUser(RequestDocument req, ApprovalMaster master) throws Exception {
		...
		QueryResult result = PersistenceHelper.manager.navigate(req, "project", ReqDocumentProjectLink.class);
		...
		while (result.hasMoreElements()) {
			...
			ArrayList<ApprovalLine> agreeUsers = ApprovalHelper.manager.getAgreeLines(master);
...
//			WTPart part = (WTPart) per;
	...
	public Map<String, Object> returnAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			ApprovalMaster master = line.getMaster();
			master.setCompleteTime(completeTime);
			master.setState(ApprovalHelper.MASTER_RETURN);
			master = (ApprovalMaster) PersistenceHelper.manager.modify(master);
	...
	public Map<String, Object> receiveAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			line.setDescription(description);
			line.setCompleteTime(completeTime);
			line.setState(ApprovalHelper.LINE_RECEIVE_COMPLETE);
			line = (ApprovalLine) PersistenceHelper.manager.modify(line);
	...
	public Map<String, Object> unagreeAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			line.setCompleteTime(completeTime);
			line.setCompleteUserID(user.getName());
			line.setState(ApprovalHelper.LINE_AGREE_REJECT);
			line = (ApprovalLine) PersistenceHelper.manager.modify(line);

			ApprovalMaster master = line.getMaster();
			master.setCompleteTime(completeTime);
			master.setState(ApprovalHelper.LINE_AGREE_REJECT);
			master = (ApprovalMaster) PersistenceHelper.manager.modify(master);
...
//			Persistable per = master.getPersist();
	...
	public Map<String, Object> agreeAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			line.setDescription(description);
			line.setCompleteTime(completeTime);
			line.setState(ApprovalHelper.LINE_AGREE_COMPLETE);
			line.setCompleteUserID(user.getName());
			line = (ApprovalLine) PersistenceHelper.manager.modify(line);
			...
			boolean isEndAgree = ApprovalHelper.manager.isEndAgree(line.getMaster());
			if (isEndAgree) {
				ArrayList<ApprovalLine> appLines = ApprovalHelper.manager.getAppLines(line.getMaster());
				for (ApprovalLine appLine : appLines) {
					int sort = appLine.getSort();
					appLine.setSort(sort - 1);
					appLine = (ApprovalLine) PersistenceHelper.manager.modify(appLine);
	...
	public Map<String, Object> setAbsenceAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			line = (ApprovalLine) rf.getReference(oid).getObject();
			line.setState(ApprovalHelper.LINE_ABSENCE);
	...
	public Map<String, Object> recoveryAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			for (String oid : list) {
				...
				ArrayList<ApprovalLine> agreeLines = ApprovalHelper.manager.getAgreeLines(master);
				for (ApprovalLine line : agreeLines) {
					// 모든 결재 라인 삭제..
					PersistenceHelper.manager.delete(line);
				}

				ArrayList<ApprovalLine> receiveLines = ApprovalHelper.manager.getReceiveLines(master);
	...
	public void deleteAllLine(ApprovalMaster master) throws WTException {
		...
		try {
			...
			if (master.getPersist() != null) {
				master = ApprovalHelper.manager.getMaster(master.getPersist());
			}
	...
	public Map<String, Object> skipApproval(Map<String, Object> param) throws WTException {
		...
		try {
			...
			ArrayList<ApprovalLine> list = ApprovalHelper.manager.getAppLines(master);
	...
	public Map<String, Object> initApprovalAction(Map<String, Object> param) throws WTException {
		...
		try {
			...
			for (String oid : list) {
				...
				ArrayList<ApprovalLine> lines = ApprovalHelper.manager.getAppLines(master);
