...
<table class="create-table">
	...
	<tr>
		...
		<td class="indent5">
			<textarea name="description" id="description" rows="10"><%=dto.getContent() != null ? dto.getContent() : ""%></textarea>
		</td>
	</tr>
	<tr>
		<th class="lb">첨부파일</th>
...
<script type="text/javascript">
	function modify() {
		...
		const url = getCallUrl("/tbom/modify");
		const oid = document.getElementById("oid").value;
		const name = document.getElementById("name");
		const description = document.getElementById("description");

		if (isNull(name.value)) {
			alert("T-BOM 제목 값은 공백을 입력 할 수 없습니다.");
			name.focus();
			return false;
		}
