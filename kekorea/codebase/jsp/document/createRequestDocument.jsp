...
// 		upload.pageStart(null, null, "primary");
	...
	dropdownFilter = function(instance, cell, c, r, source) {
	    ...
	    if (value == "GROBAL FOUNDRIES") {
	    	...
	    	return <%=CommonCodeHelper.manager.getInstallCommonCodeByName("국가핵융합연구소") %>
	    } else {
	        return source;
	    } 
	}
	...
	<!-- create header title -->
		...
		<td>
				...
				<i class="axi axi-subtitles"></i><span>의뢰서 등록</span>
				<!-- req msg -->
...
<!-- 				<input type="button" value="자가결재" id="createSelfRequestDocumentBtn" title="자가결재" class="blueBtn" data-self="true">  -->
	...
	<table class="create_table">
		...
		<tr>
			<th><font class="req">의뢰서 제목</font></th>
			<td 
				<%if(isPopup) { %> colspan="3" <%}%>>
				<input type="text" name="name" id="name" class="AXInput wid300">
			</td>
			<%
				if(!isPopup) {
			%>
			<th><font class="req">작번 템플릿</font></th>
			<td colspan="3">
		...
		%>
		<tr>
			<th><font class="req">작번 등록</font></th>
			<td colspan="3">
		        <table id="tblBackground">
	        	    <tr>
						...
						var jexcels = jexcel(document.getElementById('spreadsheet'), {
						    ...
						    columns: [
						    	{ type: 'text', title:'체크', width:30, readOnly:true },
						        { type: 'text', title:'작번유형', width:40 },
						        { type: 'dropdown', title:'거래처', width:150, source : [<%=customer%>] },
						        { type: 'dropdown', title:'설치장소', width:100, source : [<%=install%>], filter:dropdownFilter  },
						        { type: 'text', title:'막종', width:70 },
						        { type: 'text', title:'KEK작번', width:70 },
						        { type: 'text', title:'KE작번', width:70 },
						        { type: 'text', title:'USER ID', width:70 },
						        { type: 'text', title:'요구 납기일', width:70 },
						        { type: 'text', title:'작업내용', width:200 },
						        { type: 'text', title:'모델', width:70 },
						        { type: 'text', title:'발행일', width:70 },
						        { type: 'text', title:'전기 담당자', width:70 },
						        { type: 'text', title:'기계 담당자', width:70 },
						        { type: 'text', title:'SW 담당자', width:70 },
						     ],
						});
						</script>			
						</td>
