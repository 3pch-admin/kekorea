		...
		<tr>
			<td>
				<div class="header_title">
					<i class="axi axi-subtitles"></i><span>부품수정</span>
				</div>
			</td>
			<td>
				<div class="right">
...
<!-- 					<input type="button" value="버전정보" id="infoVersionBtn" title="버전정보"> -->
					...
					%>	 --%>	
					<input type="button" value="수정" id="modifyPartActionBtn" title="수정">
				<%
					if(isPopup) {
				%>
...
// 						CADAttr[] cadAttrs = CADAttr.values();
					...
					<%								
								if("PRICE".equals(key)){
					%>
									<td colspan="3"><input id="<%=key%>"  name="<%=key %>"  value="<%=intValue %>" class="AXInput wid300"></td>
					<%
								}else{
					%>
									<td colspan="3"><input id="<%=key%>"  name="<%=key %>"  value="<%=value %>" class="AXInput wid300"></td>
							<%
								}
								break;
					...
					<tr>
						...
						if("PRICE".equals(key)){						
						%>
							<td><input id="<%=key%> name="<%=key %>"  " value="<%=intValue %>" class="AXInput wid300"></td>
						<%
						} else{
						%>
						<td><input id="<%=key%>"  name="<%=key %>"  value="<%=value %>" class="AXInput wid300"></td>
						<%
						}
						%>
					<%
...
// 						CADAttr[] cadAttrs = CADAttr.values();
					...
					<tr>
						...
						if("PRICE".equals(key)){						
						%>
							<td><input type="text" id="<%=key%>"  name="<%=key %>"  value="<%=intValue %>" class="AXInput wid300"></td>
						<%
						} else{
						%>
						<td><input type="text" id="<%=key%>"  name="<%=key %>"  value="<%=value %>" class="AXInput wid300"></td>
						<%
						}
						%>
					<%
