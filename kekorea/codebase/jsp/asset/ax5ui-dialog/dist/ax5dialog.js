...
(function () {
    ...
    UI.addClass({
		 ...
		 * @example ```js var dialog = new ax5.ui.dialog(); var mask = new
		 *          ax5.ui.mask(); dialog.setConfig({ zIndex: 5000,
		 *          onStateChanged: function () { if (this.state === "open") {
		 *          mask.open(); } else if (this.state === "close") {
		 *          mask.close(); } } });
		 * 
		 * dialog.alert({ theme: 'default', title: 'Alert default', msg: theme + '
		 * color' }, function () { console.log(this); }); ```
		 */
        var ax5dialog = function ax5dialog() {
            ...
            this.config = {
                ...
                clickEventName: "click", // (('ontouchstart' in
			 ...
			 *            [config.zIndex]
			 * @param {Function}
			 *            [config.onStateChanged] - `onStateChanged` function
			 *            can be defined in setConfig method or new
			 *            ax5.ui.dialog initialization method. However, you can
			 *            us to define an event function after initialization,
			 *            if necessary
			 * @param {Object}
			 *            [config.lang]
			 * @param {String}
			 ...
			 * @param {Number}
			 *            [config.animateTime=150]
			 * @param {Number}
			 *            [config.autoCloseTime=0] - 0보다 크면 autoCloseTime 프레임후에
			 *            dialog auto close
			 * @returns {ax5dialog}
			 * @example ``` var dialog = new ax5.ui.dialog(); dialog.setConfig({
			 *          title: "app dialog title", zIndex: 5000, onStateChanged:
			 *          function () { if (this.state === "open") { mask.open(); }
			 *          else if (this.state === "close") { mask.close(); } } });
			 *          ```
			 */
            // == class body start
            this.init = function () {
			 ...
			 * @param {Object|String}
			 *            config - dialog 속성을 json으로 정의하거나 msg만 전달
			 * @param {String}
			 *            [config.theme="default"]
			 * @param {Number}
			 *            [config.width=300]
			 ...
			 * @param {Function}
			 *            [callback] - 사용자 확인 이벤트시 호출될 callback 함수
			 * @returns {ax5dialog}
			 * @example ``` myDialog.alert({ title: 'app title', msg: 'alert' },
			 *          function(){}); ```
			 */
			 ...
			 *            [callback] - 사용자 확인 이벤트시 호출될 callback 함수
			 * @returns {ax5dialog}
			 * @example ``` myDialog.confirm({ title: 'app title', msg:
			 *          'confirm', additionalContent: function () { return "<div
			 *          style='border:1px solid #ccc;border-radius:
			 *          5px;background: #eee;padding: 10px;'>추가정보</div>"; } },
			 *          function(){}); ```
			 */
			 ...
			 *            [callback] - 사용자 확인 이벤트시 호출될 callback 함수
			 * @returns {ax5dialog}
			 * @example ``` myDialog.prompt({ title: 'app title', msg: 'alert' },
			 *          function(){}); ```
			 */
